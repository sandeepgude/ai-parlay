...
def save_parlay(db: Session, sport: str, legs: list, total_odds: float,
                payout: float, ai_response: str, user_id: int = None):
    parlay = Parlay(
        sport=sport,
        legs=legs,
...
def get_recent_parlays(db: Session, sport: str, limit: int = 10):
    return (db.query(Parlay)
              .filter(Parlay.sport == sport)
...
def delete_parlay_by_id(db: Session, id: int):
